...
function Connection(options) {
  ...
  // setup socket client
  var self = this;
  var socketOptions = utils.extend({
    host: this.options.host,
    port: this.options.port,
    timeout: this.options.connectionTimeout,
    debug: function(message) {
      self.emit('log', 'socket', self.id + ':' + message)
    }
  }, this.options.socketOptions);
  this.socketClient = new Socket(socketOptions);
...
Connection.prototype.open = function(callback) {
  ...
  async.series([
    ...
    var connected = self.connected = !err;
    if (connected) self.emit('connect');
    // if there is a callback we only try once
    if (callback) return callback(err);
    if (!err) return self.connects++;
    self.emit('log', 'connection', 'connection error ', err);
    // else attempt to connect again
    if(self.options.autoReconnect) {
      self.emit('log', 'connection', 'reconnecting...' + self.reconnects++);
      setTimeout(function() {
        if (self.socketClient.writable()) return self.socketClient.disconnect();
        self.open();
...
Connection.prototype.close = function(callback) {
  this.emit('log', 'info', this.id + ':disconnecting ' + (new Date() - this.lastMessage) + 'ms' + ' ' + this.writable() + ' con:' + this.connects + ' re:' + this.reconnects);
...
Connection.prototype.onDisconnect = function() {
  // determine whether or not this was a spontaneous disconnection
  // from a connected state
  var wasConnected = this.connected;
  this.connected = false;
  this.emit('disconnect', this);
  if(!this.isShutdown && this.options.autoReconnect) {
    // if we were spontaneously disconnected, reconnect without delay
    var self = this;
    setTimeout(function() {
      self.open();
    }, (wasConnected ? 0 : self.options.retryInterval));
  }
};
...
Connection.prototype.setup = function(callback) {
  ...
  resultEmitter.on('error', this.close.bind(this, null));
...
Connection.prototype.execute = function () {
  ...
  this.emit('log', 'query', 'executing query: ' + args.query);
...
Connection.prototype.stream = function () {
  var args = utils.parseCommonArgs.apply(null, arguments);
  this.emit('log', 'query', 'Executing for streaming prepared query: 0x' + args.query.toString('hex'));
...
Connection.prototype.executePrepared = function () {
  var args = utils.parseCommonArgs.apply(null, arguments);
  this.emit('log', 'query', 'Executing prepared query: 0x' + args.query.toString('hex'));
...
Connection.prototype.prepare = function (query, callback) {
  this.emit('log', 'query', 'Preparing query: ' + query);
...
Connection.prototype.sendStream = function (frameWriter, options, callback) {
  ...
  function writeCallback(err) {
    ...
    self.emit('log', 'connection', 'Sent stream #' + frameWriter.streamId);
  }
};
...
Connection.prototype.freeStreamId = function(header) {
  ...
  if(handler && handler.callback) {
    this.emit('log', 'connection', 'callback using socket ' + this.id + ':sid:' + streamId);
    handler.callback(header.error, handler.rowLength);
  }
  this.emit('log', 'connection', 'Done receiving frame #' + streamId + ' on ' + this .id);
};
...
Connection.prototype.handleResult = function (header, err, result) {
  var streamId = header.streamId;
  if(streamId < 0) {
    return this.emit('log', 'connection', 'event received', header);
  }
  var handler = this.streamHandlers[streamId];
  if (!handler) {
    return this.emit('log', 'error', 'The server replied with a wrong streamId #' + streamId);
  }
  this.emit('log', 'connection', 'Received frame #' + streamId + ' on ' + this.id);
...
Connection.prototype.handleRow = function (header, row, fieldStream, rowLength) {
  ...
  this.emit('log', 'connection', 'Received streaming frame #' + streamId);
