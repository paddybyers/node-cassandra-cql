...
function Connection(options) {
  ...
  if (debugConnection) {
    var debugLog = this.emit.bind(this, 'log', 'debug');
    this.socketClient.debug(function(message) { debugLog('CONNECTION: ' + message); });
  }
}
 ...
 */
Connection.prototype.writable = function() { this.socketClient.writable(); };
...
Connection.prototype.open = function(callback) {
  ...
  async.series([
    ...
    self.emit('log', 'error', 'connection error ' + err.stack)
    self.emit('log', 'info', 'reconnecting...' + self.reconnects++)
    setTimeout(self.connect.bind(self), self.options.retryInterval);
  });
};

/**
 * Close the connection without trying to reconnect
 */
...
/**
 * Simple method used to reconnect on a timeout (idle socket)
 */ 
Connection.prototype.reconnect = function() {
  ...
  async.series([
    ...
    if (err) {
      self.emit('log', 'error', 'reconnect error:' + err.stack);
      self.connect();
    }
  });
};
...
Connection.prototype.setup = function(callback) {
  ...
  this.socketClient.onDisconnect = this.connect.bind(this);
  this.socketClient.onTimeout = this.reconnect.bind(this);
...
Connection.prototype.setKeyspace = function(callback) {
  if (!self.options.keyspace) return callback();
  this.execute('USE ' + self.options.keyspace + ';', null, callback);
};
