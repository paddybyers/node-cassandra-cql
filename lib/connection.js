...
Connection.prototype.writable = function() { 
  this.emit('log', 'socket', this.id + ':isShutdown:' + this.isShutdown + ':connected:' + this.connected + ':socket.writable:' + this.socketClient.writable());
...
Connection.prototype.open = function(callback) {
  ...
  async.series([
    ...
    self.emit('log', 'socket', 'connection error ', err);
    self.emit('log', 'socket', 'reconnecting...' + self.reconnects++);
    setTimeout(function() {
...
Connection.prototype.close = function(callback) {
  this.emit('log', 'socket', this.id + ':disconnecting ' + (new Date() - this.lastMessage) + 'ms' + ' ' + this.writable() + ' con:' + this.connects + ' re:' + this.reconnects);
...
Connection.prototype.sendStream = function (frameWriter, options, callback) {
  ...
  this.emit('log', 'socket', 'using socket ' + this.id);
  this.getStreamId(function getStreamIdCallback(streamId) {
    ...
    this.emit('log', 'socket', 'start using socket ' + this.id + ':sid:' + streamId);
...
Connection.prototype.freeStreamId = function(header) {
  ...
  this.emit('log', 'socket', 'done using socket ' + this.id + ':sid:' + streamId);
  if(handler && handler.callback) {
    this.emit('log', 'socket', 'callback using socket ' + this.id + ':sid:' + streamId);
...
Connection.prototype.handleResult = function (header, err, result) {
  ...
  this.emit('log', 'socket', 'result using socket ' + this.id + ':sid:' + streamId);
